<template>
  <div class="preview-root">
    ...
    <div v-if="loading" class="loading-container">
      ...
      <p>Loading projects hero section...</p>
    </div>
    ...
    <div v-else-if="error" class="error-container">
      ...
      <h3>Failed to load projects hero data</h3>
      <p>{{ error }}</p>
...
const heroData = ref({
  badge: 'Our Projects',
  title: 'BUILDING <span class="text-industrial-blue">BANGLADESH</span>',
  description: 'From mega power projects to renewable energy installations, we deliver engineering excellence that powers the nation.'
})
...
const fetchData = async () => {
  try {
    loading.value = true
    error.value = null
    const response = await axios.get(`${API_CONFIG.baseURL}${API_ENDPOINTS.PROJECTS_HERO}`)
    if (response.data && response.data.success && response.data.data) {
      heroData.value = response.data.data
    }
  } catch (err) {
    console.warn('Projects hero API error, using defaults:', err.message)
