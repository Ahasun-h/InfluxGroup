<template>
  <div class="preview-root">
    ...
    <div v-if="loading" class="loading-container">
      ...
      <p>Loading products hero section...</p>
    </div>
    ...
    <div v-else-if="error" class="error-container">
      ...
      <h3>Failed to load products hero data</h3>
      <p>{{ error }}</p>
...
const heroData = ref({
  badge: 'Our Products',
  title: 'ENGINEERING <span class="text-industrial-blue">EXCELLENCE</span>',
  description: 'Comprehensive portfolio of power systems and equipment designed for reliability, efficiency, and sustainability.'
})
...
const fetchData = async () => {
  try {
    loading.value = true
    error.value = null
    const response = await axios.get(`${API_CONFIG.baseURL}${API_ENDPOINTS.PRODUCTS_HERO}`)
    if (response.data && response.data.success && response.data.data) {
      heroData.value = response.data.data
    }
  } catch (err) {
    console.warn('Products hero API error, using defaults:', err.message)
    // Keep the default values — don't show error state for missing API
  } finally {
    loading.value = false
  }
}
...
/* Allow v-html span to carry text-industrial-blue colour */
