...
class ProjectsPageController extends Controller
{
    public function index(): View
    {
        ...
        return view('admin.cms-section.projects-page', compact('cmsData'));
    }
    ...
    private function updateOrCreate(string $sectionName, string $itemName, array $values): ContentManagement
    {
        ...
        if ($item) {
            $item->update(array_merge(['page_name' => 'projects_page'], $values));
            return $item;
        }
    ...
    public function update(Request $request, string $section): JsonResponse
    {
        try {
            ...
            Log::error("Failed to update Projects Page section ({$section}): " . $e->getMessage());
    ...
    private function updateHeroSection(Request $request): JsonResponse
    {
        if ($request->has('badge')) {
            $this->updateOrCreate('projects_hero', 'projects_hero_badge', ['section_content' => $request->badge]);
        }
        if ($request->has('title')) {
            $this->updateOrCreate('projects_hero', 'projects_hero_title', ['section_content' => $request->title]);
        }
        if ($request->has('description')) {
            $this->updateOrCreate('projects_hero', 'projects_hero_description', ['section_content' => $request->description]);
        }

        return response()->json(['success' => true, 'message' => 'Projects Hero section updated successfully.']);
    }
}
